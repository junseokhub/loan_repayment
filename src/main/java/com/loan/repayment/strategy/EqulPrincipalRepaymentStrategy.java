...
public class EqulPrincipalRepaymentStrategy implements RepaymentStrategy {
    ...
    public List<RepaymentPlan> calculate(LoanRequest request) {
        ...
        BigDecimal equalPrincipal = principal.divide(BigDecimal.valueOf(n), 0, RoundingMode.FLOOR);
        BigDecimal remainder = principal.subtract(equalPrincipal.multiply(BigDecimal.valueOf(n)));

        List<RepaymentPlan> schedules = new ArrayList<>();
        BigDecimal remaining = principal;

        for (int i = 1; i <= n; i++) {
            BigDecimal interest = remaining.multiply(r).setScale(0, RoundingMode.FLOOR);
